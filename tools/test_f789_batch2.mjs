...
class El {
  constructor(tag = 'button', attrs = '') {
    this.tagName = tag.toUpperCase(); this.textContent = ''; this.innerHTML = ''; this.onclick = null; this.children = [];
    this.dataset = {}; this.classList = { toggle() {} };
    for (const m of attrs.matchAll(/data-([a-zA-Z0-9-]+)="([^"]*)"/g)) this.dataset[m[1].replace(/-([a-z])/g, (_, c) => c.toUpperCase())] = m[2];
    const id = attrs.match(/\bid="([^"]+)"/); if (id) this.id = id[1];
  }
...
const document = { getElementById(id){ return nodes[id] || byId.get(id); }, querySelector(selector){ return matches(selector)[0] || null; }, querySelectorAll(selector){ return matches(selector); } };
...
const clickNth = (selector, index, label) => { const list = matches(selector); const b = list[index]; if (!b) throw new Error(`Button not found: ${label}`); b.click(); };
...
reset(); click('[data-v="5"]', '5'); byId.get('shift').click(); clickNth('[data-v="÷"]', 1, 'Shift numeric ÷ / nCr'); click('[data-v="2"]', '2'); byId.get('equals').click(); assertEq(result(), '10', 'nCr');
